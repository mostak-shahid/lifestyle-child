...
$animation = $mosacademy_options['sections-service-animation'];
$animation_delay = $mosacademy_options['sections-service-animation-delay'];
$title = $mosacademy_options['sections-service-title'];
$content = $mosacademy_options['sections-service-content'];
$layout = $mosacademy_options['sections-service-layout'];
$gap = $mosacademy_options['sections-service-gap'][1];
$slides = $mosacademy_options['sections-service-slides'];
$view = $mosacademy_options['sections-service-view'];
			...
			<?php do_action( 'action_before_service_loop', $page_details ); ?>
				...
				<div class="<?php if ($view == 'grid') echo 'col-md-'.$smallcol.' col-lg-'.$colsize; else echo 'wrapper'?><?php if ($gap) echo ' mt15 mb15'; else echo ' no-padding'?>">
					<div class="service-unit">
						<div class="img-part">
							<?php if ($slide['image']) : ?>
								<img class="img-responsive img-fluid img-service-one" src="<?php echo wp_get_attachment_url( $slide['attachment_id'] ) ?>" alt="<?php echo strip_tags(do_shortcode( $slide['title'] )) ?> | Sydney" width="<?php echo $slide['width'] ?>" height="<?php echo $slide['height'] ?>">
							<?php endif; ?>
							<?php if ($slide['photo']) : ?>
								<img class="img-responsive img-fluid img-service-two" src="<?php echo wp_get_attachment_url( $slide['photo_attachment_id'] ) ?>" alt="<?php echo strip_tags(do_shortcode( $slide['title'] )) ?> | Sydney" width="<?php echo $slide['photo_width'] ?>" height="<?php echo $slide['photo_height'] ?>">
							<?php endif; ?>
						</div>
						<div class="content">	
							<div class="wrapper">					
